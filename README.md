...
1. APIs should validate against the same schemas as the UI
2. Refactor APIs for better semantics
3. Error handling (redirect to 404 properly)
4. Hitting the "continue" button on a step should return to the task list page if the form submitted successfully
5. Refactor forms to use formik status
6. Improve autosave so it doesn't submit after initial form mount
7. Write a script to convert a CSV into form config
8. **Submit** to the case viewer api
